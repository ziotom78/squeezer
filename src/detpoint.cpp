...
public:
    ...
    void read_column(const std::string & column_name,
...
Detector_pointings_t::read_from_database(const std::string & obj_name)
{
	...
	detpoint_obj.read_column("sampleOBT", obt_times);
	detpoint_obj.read_column("sampleSCET", scet_times);
	detpoint_obj.read_column("theta", theta);
	detpoint_obj.read_column("phi", phi);
	detpoint_obj.read_column("psi", psi);
    }
